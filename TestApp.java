...
public class TestApp {
	...
	public TestApp(	MainWindow mainwindow,
					String command, 
					String inputFile, 
					String outputFile, 
					String language) {
		...
		try {
			if (mode.equalsIgnoreCase("help")) {
				displayDetailedHelp(mode);
			} else if (mode.equalsIgnoreCase("recognize")) {
				performRecognition(mw,inputFile, language, outputFile);
			} 
...
//			} 
	 ...
	 */
	private static void performRecognition(	MainWindow mw,
											String inputFile, 
											String lang, 
											String outputFile)
			throws Exception {
...
//			mw.runocrarea.append("Uploading file...\n");
			task = restClient.processImage(inputFile, settings);


		waitAndDownloadResult(mw, task, outputPath);
	}
	 ...
	 * vCard, csv or xml
	 */
	private static void performBusinessCardRecognition(MainWindow mw,Vector<String> argList)
			throws Exception {
	...
	private static void performTextFieldRecognition(MainWindow mw, Vector<String> argList)
			throws Exception {
		String language = CmdLineOptions.extractRecognitionLanguage(argList);
	...
	private static void performBarcodeRecognition(MainWindow mw, Vector<String> argList)
			throws Exception {
		String outputPath = argList.lastElement();
		argList.remove(argList.size() - 1);
		// argList now contains list of source images to process
	 ...
	 */
	private static void performFieldsRecognition(MainWindow mw, Vector<String> argList)
			throws Exception {
	...
	private static void performMrzRecognition(MainWindow mw,Vector<String> argList)
			throws Exception {
		String outputPath = argList.lastElement();
		argList.remove(argList.size() - 1);
		// argList now contains list of source images to process
	 ...
	 */
	private static void waitAndDownloadResult(MainWindow mw, Task task, String outputPath)
			throws Exception {
		task = waitForCompletion(task);

		if (task.Status == Task.TaskStatus.Completed) {
			System.out.println("Downloading..");
			//mw.runocrarea.append("Downloading...\n");
			restClient.downloadResult(task, outputPath);
			System.out.println("Done");
			//mw.runocrarea.append("Done\n");
			...
			//mw.runocrarea.append("Task failed\n");
		}
